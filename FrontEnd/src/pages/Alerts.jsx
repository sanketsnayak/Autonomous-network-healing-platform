  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">Monitor and manage network alerts and incidents</p>
      </div>
        ...
        {filteredAlerts.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-200">
            {filteredAlerts.map((alert) => (
              <li key={alert._id} className="px-6 py-4 hover:bg-gray-50">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedAlert.device_ip}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <span className={`mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSeverityColor(selectedAlert.severity)}`}>
                      {selectedAlert.severity}
                    </span>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Status</label>
                    <span className={`mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(selectedAlert.status)}`}>
                      {selectedAlert.status}
                    </span>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Category</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedAlert.category}</p>
                  </div>
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-gray-700">Timestamp</label>
                  <p className="mt-1 text-sm text-gray-900">{formatDateTime(selectedAlert.timestamp)}</p>
                </div>
                
                {selectedAlert.normalized_data && (
                  <div>
                    ...
                    <pre className="mt-1 text-sm text-gray-900 bg-gray-50 p-2 rounded">
                      {JSON.stringify(selectedAlert.normalized_data, null, 2)}
                    </pre>
                  </div>
                )}
              </div>
